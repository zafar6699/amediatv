...
ezoom = {
	...
	createZoomModalTag: function () {
		...
		var $ezoomWrap = $(
				'<div id="zoomModal" class="modal" style="display:none;position:fixed;z-index: 1000000;padding:0px 0;left:0;top:0;width:100%;height:100%;overflow:auto;background-color:rgba(0,0,0,0.8);transition:.3s"></div>'
			),
			$closeBtn = $(
			...
			$rotateRightBtn = $(
				'<span title="Rotate right" class="fas fa-redo" id="close" style="z-index:1;cursor:pointer;position:fixed;top:15px;right:70px;color:#b5b5b5;font-size:20px;font-weight:700;transition:.3s"></span>'
			),
			$rotateLeftBtn = $(
				'<span title="Rotate left" class="fas fa-undo" id="close" style="z-index:1;cursor:pointer;position:fixed;top:15px;right:105px;color:#b5b5b5;font-size:20px;font-weight:700;transition:.3s"></span>'
			),
			$image = $(
				'<img title="Please scroll the mouse pointer to zoom in/out the image." class="" style="border-radius:5px;cursor:move;margin:auto;display:block;max-width:100%;max-height:100%" id="zoomModalImg">'
			)
	...
	show: function (src) {
		...
		if (
			typeof ezoom.options.onShow &&
			typeof ezoom.options.onShow == "function"
		) {
	...
	remove: function (op) {
		...
		if (
			typeof ezoom.options.onClose &&
			typeof ezoom.options.onClose == "function"
		) {
	...
	doRotate: function (to) {
		...
		ezoom.setTransformOrigin(1, null, 0, 0)
		if (
			typeof ezoom.options.onRotate &&
			typeof ezoom.options.onRotate == "function"
		) {
	...
	initMouseScrolling: function () {
		...
		if (document.attachEvent) {
			...
			ezoom.zoomModal[0].addEventListener(
				MOUSEWHEEL_EVENT,
				function (e) {
					mouseWheelScroll(e)
				},
	...
	initEvent: function () {
		...
		ezoom.zoomModal.on("mousedown", function (e) {
			...
			if (
				typeof ezoom.options.onMoveStarted &&
				typeof ezoom.options.onMoveStarted == "function"
			) {
		...
		$(document).on("mouseup", function () {
			...
			translateY_ = ezoom.translateY
			if (
				ezoom.options.onMovedCompleted &&
				typeof ezoom.options.onMovedCompleted == "function"
			) {
		...
		$(document).on("mousemove", function (e) {
			...
			if (ezoom.grabbing) {
				...
				if (
					typeof ezoom.options.onMoving &&
					typeof ezoom.options.onMoving == "function"
				) {
